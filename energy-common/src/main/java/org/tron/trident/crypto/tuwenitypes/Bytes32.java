...
public interface Bytes32 extends Bytes {
  ...
  static Bytes32 wrap(byte[] bytes) {
    ...
    Preconditions.checkArgument(bytes.length == SIZE, "Expected %s bytes but got %s", SIZE, bytes.length);
    return wrap(bytes, 0);
  }
  ...
  static Bytes32 wrap(Bytes value) {
    ...
    Preconditions.checkArgument(value.size() == SIZE, "Expected %s bytes but got %s", SIZE, value.size());
  ...
  static Bytes32 leftPad(Bytes value) {
    ...
    Preconditions.checkArgument(value.size() <= SIZE, "Expected at most %s bytes but got %s", SIZE, value.size());
