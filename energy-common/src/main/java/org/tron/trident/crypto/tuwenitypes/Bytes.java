...
public interface Bytes extends Comparable<Bytes> {
  ...
  static Bytes of(int... bytes) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      int b = bytes[i];
      Preconditions.checkArgument(b == (((byte) b) & 0xff), "%sth value %s does not fit a byte", i + 1, b);
      result[i] = (byte) b;
    }
  ...
  static Bytes fromHexStringLenient(CharSequence str, int destinationSize) {
    Preconditions.checkNotNull(str);
    Preconditions.checkArgument(destinationSize >= 0, "Invalid negative destination size %s", destinationSize);
  ...
  default int toInt(ByteOrder order) {
    int size = size();
    Preconditions.checkArgument(size <= 4, "Value of size %s has more than 4 bytes", size());
    if (size == 0) {
      return 0;
    }
    if (order == BIG_ENDIAN) {
  ...
  default long toLong(ByteOrder order) {
    int size = size();
    Preconditions.checkArgument(size <= 8, "Value of size %s has more than 8 bytes", size());
    if (size == 0) {
      return 0;
    }
    if (order == BIG_ENDIAN) {
  ...
  default void copyTo(MutableBytes destination, int destinationOffset) {
    ...
    Preconditions.checkElementIndex(destinationOffset, destination.size());
    checkArgument(
